...
struct SpMV {
	...
	SpMV(int_type *x_, int_type *y_, real_type *val_, int_type num_rows_, int_type num_cols_, int_type num_nnz_, real_type *vec_, int block_size_1d = DEFAULT_BLOCK_SIZE_1D, int debug=0, GPU_IMPL gpu_impl=GPU_IMPL(0), bool use_half_precision_gpu=DEFAULT_USE_HALF_PRECISION_GPU) :
			x(x_), y(y_), val(val_), num_rows(num_rows_), num_cols(num_cols_), num_nnz(num_nnz_), vec(vec_), block_size_1d(block_size_1d), gpu_impl(GPU_IMPL(gpu_impl)), use_half_precision_gpu(use_half_precision_gpu) {
	...
	void operator()(int debug) {
		...
		cudaMemcpyAsync(res, res_d, sizeof(real_type) * num_rows, cudaMemcpyDefault);
...
long sw_test(coo_t<I, V> &coo, V *vec, V *sw_res) {
	auto start = clock_type::now();
	spmv_coo_gold(coo, sw_res, vec);
...
int main(int argc, char *argv[]) {
	...
	auto sw_time = sw_test(coo, vec, sw_res.data());
	...
	SpMV spmv(coo.start.data(), coo.end.data(), coo.val.data(), rows, cols, nnz, vec, block_size_1d, debug, GPU_IMPL(2), options.use_half_precision_gpu);
	...
	for (uint i = 0; i < num_tests; i++) {
		...
		if (reset) {
			...
			std::fill(sw_res.begin(), sw_res.end(), 0);
			sw_time = sw_test(coo, vec, sw_res.data());	
		}
		...
		int error = check_array_equality(hw_res.data(), sw_res.data(), rows, 10e-6, debug);
