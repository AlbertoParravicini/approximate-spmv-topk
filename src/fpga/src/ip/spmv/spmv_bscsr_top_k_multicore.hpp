...
inline void inner_spmv_topk_product_stream(
	...
	static real_type pointwise_res_local[BSCSR_PACKET_SIZE];
...
#pragma HLS pipeline II=hls_pipeline
        ...
        RESET_BUFFERS: for (x_bscsr j = 0; j < LIMITED_FINISHED_ROWS + 1; j++) {
#pragma HLS unroll
        ...
        READ_AGG_AND_FINISHED_ROW_CHECK: for (x_bscsr j = 1; j < LIMITED_FINISHED_ROWS; j++) {
#pragma HLS unroll
        ...
        for (x_bscsr j = 0; j < LIMITED_FINISHED_ROWS + 1; j++) {
#pragma HLS unroll
...
//  }
    ...
    WRITE_LOCAL_2: for (x_bscsr i = 0; i < LIMITED_FINISHED_ROWS; i++) {
#pragma HLS unroll
