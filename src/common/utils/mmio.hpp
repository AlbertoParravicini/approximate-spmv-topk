...
template<typename I>
int mm_read_mtx_crd_size(FILE *f, I *M, I *N, I *nz);
template<typename I>
int mm_read_mtx_array_size(FILE *f, I *M, I *N);

int mm_write_banner(FILE *f, MM_typecode matcode);
template<typename I>
int mm_write_mtx_crd_size(FILE *f, I M, I N, I nz);
template<typename I>
int mm_write_mtx_array_size(FILE *f, I M, I N);
...
template<typename I>
inline int mm_write_mtx_crd_size(FILE *f, I M, I N, I nz) {
	if (fprintf(f, "%u %u %u\n", M, N, nz) != 3)
...
template<typename I>
inline int mm_read_mtx_crd_size(FILE *f, I *M, I *N, I *nz) {
	char line[MM_MAX_LINE_LENGTH];
	int num_items_read;
...
template<typename I>
inline int mm_read_mtx_array_size(FILE *f, I *M, I *N) {
	char line[MM_MAX_LINE_LENGTH];
	int num_items_read;
	/* set return null parameter values, in case we exit with errors */
	*M = *N = 0;
...
template<typename I>
inline int mm_write_mtx_array_size(FILE *f, I M, I N) {
	if (fprintf(f, "%u %u\n", M, N) != 2)
		return MM_COULD_NOT_WRITE_FILE;
	else
		return 0;
...
inline char *mm_typecode_to_str(MM_typecode matcode) {
	...
	char *buffer2 = reinterpret_cast<char *>(malloc((len + 1) * sizeof(char)));
	snprintf(buffer2, len + 1, "%s", buffer);
	return buffer2;
}
